...
- [x] Decide PostgreSQL version path: 18+ (native) vs &lt; 18 (custom `uuidv7()`). Schema comment documents both; use `app/db/uuid7_function.sql` for PG &lt; 18.
- [x] If PG &lt; 18: ensure `uuid7_function.sql` is run before table creation (documented in `schema.sql`).
- [x] In `schema.sql`: replace all `DEFAULT uuid_generate_v4()` with `DEFAULT uuidv7()`.
- [x] In `schema.sql`: replace all `DEFAULT gen_random_uuid()` with `DEFAULT uuidv7()`.
- [x] In `trigger.sql`: replace all `uuid_generate_v4()` with `uuidv7()`.
- [x] In `archival_config_table.sql`: replace all `uuid_generate_v4()` with `uuidv7()`.
- [ ] Review `index.sql`: remove or adjust indexes that only supported `created_date` ordering (optional).
- [ ] Rebuild DB (tear down + run `uuid7_function.sql` if PG &lt; 18, then schema, index, trigger, seed).
- [ ] Verify: `SELECT uuidv7();` and insert into a table, confirm PK is UUID7 (version 7).

### Application code (after schema is done)

- [ ] Update default ordering in `crud_service.py` to use ID where appropriate (optional).
...
- [x] Ensure all API schemas that accept UUIDs use a type that accepts UUID7: Schemas changed from `UUID4` to `UUID` where applicable.
...
**Status**: Schema and trigger changes applied; payment method schemas updated to accept UUID (not only UUID4). Rebuild DB (run `uuid7_function.sql` first if PostgreSQL &lt; 18) and run tests.
