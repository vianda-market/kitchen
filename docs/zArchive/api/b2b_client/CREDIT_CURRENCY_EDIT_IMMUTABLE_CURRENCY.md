...
**Date**: February 2026  
**Purpose**: Backend guardrails for `PUT /api/v1/credit-currencies/{id}` so only **credit_value_local_currency** and **status** are editable; currency identity (`currency_name`, `currency_code`) is immutable after creation.  
...
- **Editable on update**: `credit_value_local_currency`, `status` (and any other lifecycle/attribute fields the API already allows).
...
3. **Editable vs identity**  
   - **Editable**: `credit_value_local_currency` (conversion rate / value per credit), `status` (e.g. Active / Inactive). Same logical currency, clear audit: “credit_value_local_currency changed from 1.0 to 1.5” or “status changed to Inactive.”  
...
- **Accept for update**: `credit_value_local_currency`, `status` (and any other fields that are explicitly defined as updatable in your API spec).
...
- **Option B – Reject**: If `currency_name` or `currency_code` is present in the body, return `400 Bad Request` with a message such as: `"currency_name and currency_code cannot be changed on an existing credit currency; only credit_value_local_currency and status are editable."`
...
- On **edit**: Show `currency_name` (and `currency_code` if displayed) as **read-only** in the form. Only `credit_value_local_currency` and `status` are editable in the UI.
- On **PUT**: Send only `credit_value_local_currency` and `status`. It will **not** send `currency_name` or `currency_code` on update.
...
- Create: `POST /api/v1/credit-currencies/` with `currency_name` + `credit_value_local_currency` (no `currency_code` on create).
