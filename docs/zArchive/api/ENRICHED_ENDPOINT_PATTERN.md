...
- `GET /users/{user_id}` → Returns `UserResponseSchema` with `role_id` and `institution_id` (UUIDs)

### Enriched Endpoints (New Pattern)
- `GET /users/enriched/` → Returns `UserEnrichedResponseSchema` with `role_name`, `role_type`, and `institution_name`
- `GET /users/enriched/{user_id}` → Returns `UserEnrichedResponseSchema` with `role_name`, `role_type`, and `institution_name`
...
### User Management
- `GET /users/enriched/` - Users with role and institution names
- `GET /users/enriched/{user_id}` - Single user with role and institution names
