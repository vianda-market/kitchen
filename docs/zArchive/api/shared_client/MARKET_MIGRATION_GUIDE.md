...
- `GET /api/v1/discretionary/enriched/` - List discretionary requests with market details. Responses include `created_by` (user_id of who requested the credit) and `created_by_name` (display name) for the "Created by" table column.
...
interface PlanEnriched {
  ...
  country_code: string;        // NEW (e.g., "AR" – ISO 3166-1 alpha-2)
  credit_currency_id: string;
  currency_name: string;
  currency_code: string;
  name: string;
...
interface SubscriptionEnriched {
  ...
  country_code: string;         // NEW (alpha-2)
  renewal_date: string;
  balance: number;
  is_archived: boolean;
  status: string;
...
interface DiscretionaryEnriched {
  ...
  user_username: string | null;   // Recipient username (Customer requests)
  ...
  comment: string | null;
  status: string;
  created_by: string | null;      // user_id of who requested the discretionary credit (from history)
  created_by_name: string | null; // display name for "Created by" column in table
}
```
