...
This document describes **GET /api/v1/plate-pickup/pending** — the endpoint that returns the current user’s **single pending pickup order** (or `null`). It does **not** describe how **plate selection** (booking plates) works. For creating and listing plate selections, see [PLATE_SELECTION_API.md](PLATE_SELECTION_API.md).
...
- **Plate selection (book a plate):** [PLATE_SELECTION_API.md](PLATE_SELECTION_API.md) — POST /plate-selections/, GET /plate-selections/
