...
3. All other updatable profile fields (email, first_name, last_name, mobile_number (E.164), role_name, market_id / market_ids, employer_id, status, etc.) remain editable by admins as allowed by the API and role rules. Responses include read-only `mobile_number_verified` and `mobile_number_verified_at`; changing `mobile_number` resets verification to false.
