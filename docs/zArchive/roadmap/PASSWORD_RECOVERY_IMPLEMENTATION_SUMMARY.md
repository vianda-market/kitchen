...
**Source of truth for password recovery.** Originally planned as "Next Sprint" after the FastAPI upgrade (see archived `IMPLEMENTATION_SUMMARY.md`). All password recovery work is tracked here; the optional seed-data test fix and FastAPI upgrade details remain in that archived doc.
