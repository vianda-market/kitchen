...
- [x] Add `X-Deprecated-Endpoint` header to responses for self-updates ✅ **Implemented** — `X-Deprecated-Endpoint`, `X-Use-Instead`, `X-Deprecation-Date`, `X-Removal-Date` on GET/PUT `/{user_id}` and GET `/enriched/{user_id}` when self-read/self-update
- [ ] Return deprecation notice in response body (Optional - deferred)

### Phase 3: Update Documentation (Immediate)
- [x] Update API documentation to recommend `/me` endpoints for ALL users (self-updates) ✅ **See** [USER_SELF_UPDATE_PATTERN.md](../api/shared_client/USER_SELF_UPDATE_PATTERN.md)
- [x] Document access pattern matrix (who uses what endpoints) ✅ **See** USER_SELF_UPDATE_PATTERN.md "Usage Patterns by Role"
- [x] Update Postman collections to use `/me` endpoints for self-updates ✅ **Audited** — E2E, Permissions, ROLE AND FIELD ACCESS use `/me` for self-reads/updates; `/{user_id}` used only for admin operations. See [postman/README.md](../postman/README.md) "User Endpoint Pattern"
- [ ] Update vianda-platform client to use `/me` for Supplier Admin self-updates
- [x] Add migration guide for API consumers ✅ **See** USER_SELF_UPDATE_PATTERN.md "Migration Guide"
- [x] Update OpenAPI/Swagger docs with deprecation notices ✅ **FastAPI** `deprecated=True` on path-parameter endpoints

### Phase 4: Monitor Usage (1-2 months)
- [x] Add metrics/logging to track usage of deprecated endpoints ✅ **Implemented** — `log_deprecated_endpoint_usage()` in `app.utils.log`; logs "DEPRECATED ENDPOINT USAGE" with endpoint, user_id, role_type, timestamp for grep/monitoring
- [ ] Monitor for self-update usage of deprecated endpoints (all user types) — grep logs for `DEPRECATED ENDPOINT USAGE`
...
- [x] Add tests for deprecation warnings ✅ **Implemented** — `test_deprecated_get_user_self_read_returns_x_deprecated_header`, `test_deprecated_put_user_self_update_returns_x_deprecated_header`, `test_deprecated_get_enriched_user_self_read_returns_x_deprecated_header` verify X-Deprecated-Endpoint header on self-read/self-update
