...
- Route `POST /api/v1/addresses/validate` removed
- `AddressValidationService` removed; gateway `validate_address` removed
- `map_validation_result_to_address`, `build_validation_api_request` removed from mapping
- Address Validation mocks removed from `address_autocomplete_mocks.json`
- Test classes `TestMapValidationResultToAddress`, `TestBuildValidationApiRequest` removed
- Docstrings updated in `address_autocomplete_service.py` and `google_places_gateway.py`
- "Address Validate – Province for Google API" section removed from `PROVINCES_API_CLIENT.md`
