...
if (pm.response.code === 200 || pm.response.code === 201) {
    ...
    pm.collectionVariables.set("entityBillId", response.institution_bill_id);
    console.log("✅ Institution bill created successfully");
    console.log("Bill ID:", response.institution_bill_id);
    console.log("Amount:", response.amount);
    console.log("Status:", response.status);
} else {
    console.log("❌ Bill creation failed");
    pm.collectionVariables.set("entityBillId", "BILL_CREATION_FAILED");
}
```
...
### 38. GET Get Bills
**URL:** `{{baseUrl}}/institution-bills/?institution_id={{institutionEntityId}}`
**Headers:**
```
Authorization: Bearer {{authToken}}
```
...
**URL:** `{{baseUrl}}/institution-payment-attempts/minimal`
**Headers:**
```
Content-Type: application/json
Authorization: Bearer {{authToken}}
...
```json
{
  ...
  "amount": 42000,
  "currency_code": "{{planCreditCurrencyCode}}"
}
```
**Pre-Request Script:** `[TO BE FILLED]`
**Post-Request Script:** `[TO BE FILLED]`
