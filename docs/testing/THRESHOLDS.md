...
| `maintainability-max-drop` | 10 | 2026-05-10 | Percentage-point drop in radon MI that fails the gate — caught restructures that made files materially harder to maintain. Loosened from 5 in fix/issue-128 (live.py +50 lines of validation code). Loosened 8→10 in PR #276 (maps.py adding /city-pins for the interactive-map cutover, vianda-app#214). Restore to 5 once underlying complexity is split. | `scripts/check_maintainability.sh` (`MAX_DROP`) |
