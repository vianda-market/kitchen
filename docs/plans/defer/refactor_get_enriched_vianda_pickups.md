...
1. **Extract the COUNT path into a sibling function** (`count_enriched_vianda_pickups`) that shares the joins-fragment with the list path. The COUNT-vs-list branch in the current function is responsible for ~5 of the new complexity points.
...
- Remove `app/services/entity_service.py:get_enriched_vianda_pickups` from `.complexity-baseline.txt`.
