...
2. **Restaurant prepares the plate** — may take minutes
3. **Restaurant hands off the plate** — physical handoff happens
...
|--------|-------------|-----|---------|
| **Pending** | System | Plate selection promoted to pickup | Order exists, customer hasn't shown up |
| **Arrived** | Customer (B2C) | Scans QR code | Customer is at the restaurant. Timer starts. |
| **Handed Out** | Restaurant (B2B kiosk) | Code verification OR timer expiry OR manual mark | Restaurant has given the plate to the customer |
| **Completed** | Customer (B2C) | Taps "I received my plate" OR auto-completes after grace period | Customer confirms receipt. Immutable final state. |

### Why separate "Handed Out" from "Completed"?

- **Arrived ≠ Handed Out**: The plate may need preparation after the customer arrives. Scanning a QR code doesn't mean the food is ready.
- **Handed Out ≠ Completed**: The restaurant handing off a plate doesn't mean the customer got the right order. Customer confirmation closes the loop.
- **Analytics**: We can measure prep time (Arrived → Handed Out) and confirmation delay (Handed Out → Completed) separately.

### Status enum impact

`status_enum` in PostgreSQL needs a new value: `'Handed Out'`. This is used by `plate_pickup_live` only (via the `plate_pickup` context). Other tables using `status_enum` are unaffected because they use different context subsets.

The Python `Status` enum gets `HANDED_OUT = "Handed Out"`, and the `plate_pickup` context becomes: `[Pending, Arrived, Handed Out, Completed, Cancelled]`.
...
| **Manual mark** | Restaurant clerk (kiosk) | Clerk taps "Mark as handed out" on an Arrived order | `POST /plate-pickup/{id}/hand-out` |
| **Timer expiry** | B2C app | All countdown extensions exhausted, timer hits 0 | `POST /plate-pickup/{id}/complete` with `completion_type=timer_expired` |
...
## 5. Customer Confirmation ("I Received My Plate")

After status reaches "Handed Out," the B2C app should prompt the customer to confirm:

- B2C shows a card: "Did you receive your plate at [Restaurant Name]?"
- Customer taps "Yes" → `POST /plate-pickup/{id}/complete` with `completion_type=user_confirmed` → **Completed**
...
|---|---|
| **Server-authoritative timer** | `expected_completion_time` already stored on plate_pickup_live. Add to daily-orders response along with `countdown_seconds` from settings. |
...
| **Daily orders enhancements** | Add `expected_completion_time`, `completion_time`, `was_collected`, `plate_pickup_id`, `extensions_used`, `pickup_type` to response. |
...
For timer-mode restaurants, the kiosk uses `POST /plate-pickup/{id}/hand-out` to manually mark individual orders.
...
- Add `extensions_used INTEGER DEFAULT 0` to `plate_pickup_live`.
- Add `POST /api/v1/plate-pickup/{id}/extend` (B2C endpoint) that increments `extensions_used`, updates `expected_completion_time += countdown_seconds`, and checks `extensions_used < max_extensions`.
...
- View customer feedback (plate reviews)
- View their own profile

### What Operator cannot do:
- CRUD on products, plans, plates, restaurants, QR codes
...
5. **Partial handoff**: If a customer has 2 plates and the restaurant hands off 1 first, do we support per-plate handoff? Or is it all-or-nothing per confirmation code? (Recommendation: all-or-nothing per confirmation code for simplicity.)
...
|---|---|---|
| **Phase 1** (this PR) | Enhanced daily-orders response (timer fields, pickup window, server_time, plate_pickup_id). `extensions_used` column (always 0). Supplier Operator role restrictions. | Nothing |
...
| **Phase 3** | `POST /plate-pickup/{id}/extend` (B2C). Real `extensions_used` tracking. B2C customer confirmation prompt ("I received my plate"). | Phase 2 |
