...
7. ~~**import-linter layered architecture contract**~~ DONE — `.importlinter` contracts 4 (`layers`: `routes → services → gateways → dto`) and 5 (`forbidden`: `app.dto → app.db`). 10 pre-existing cross-layer imports baselined via `ignore_imports` (9 in `app.gateways.ads.*` reaching `app.services.ads.*`, plus the deferred `route_factory -> routes.restaurant`). Enforced by the existing `layers` CI job in `.github/workflows/ci.yml`.
8. ~~**ruff function-size rules (PLR0912/PLR0913/PLR0915)**~~ DONE — `pyproject.toml`. Enabled at ruff defaults (12 branches, 5 args, 50 statements — matches CLAUDE.md `<50 lines` rule). 66 existing files baselined via `[tool.ruff.lint.per-file-ignores]` (107 PLR0913 + 24 PLR0912 + 28 PLR0915 violations). Reuses the existing `lint` CI job — no workflow change needed.
9. ~~**pip-licenses compliance gate**~~ DONE — `scripts/check_licenses.py` wired into the existing `security` CI job in `.github/workflows/ci.yml`. Allowlist: MIT, Apache-2.0, BSD-*, ISC, MPL-2.0, PSF, Unlicense. Fails on GPL/AGPL/LGPL/UNKNOWN. Five packages individually allowlisted with justifications: `google-crc32c`/`google-ads`/`facebook_business` (ambiguous metadata), `psycopg2-binary`/`pycountry` (LGPL, dynamic-linked per LGPL §6). Current run: 83 packages, OK.
10. ~~**vulture dead-code detection**~~ DONE — `scripts/check_vulture.sh` + `.vulture-baseline.txt` baseline. Enforced by the `deadcode` CI job in `.github/workflows/ci.yml`. Local update: `bash scripts/check_vulture.sh --update`.
11. ~~**C.R.A.P. analysis tool**~~ DONE — `scripts/analyze_crap.py`. Joins radon CC with pytest-cov per-function coverage to rank functions by `cc² × (1 − cov)³ + cc`. Not a CI gate — run ad-hoc to pick refactor targets. First run: 1695 functions analyzed, 268 risky (CRAP > 30), 97 priority (CRAP > 100). Top targets cluster in `app/services/entity_service.py`, `app/services/billing/institution_billing.py`, and `app/routes/restaurant.py`.

## Deferred (lower ROI for this repo)

- **Type coverage dashboard** — mypy baseline already enforces drift; a dashboard would only be vanity unless tied to an explicit SLO to shrink the 1602-entry baseline.
- **PR preview deploys** — Newman already validates the API contract in CI. Preview environments pay off in frontend repos (`vianda-platform`, `vianda-app`) where reviewers need to see visual changes; for this backend the incremental value is small.
