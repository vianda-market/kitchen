...
1. **Make `province` required for USA, BRA, CAN, MEX**
...
**UI Recommendation:** Make `province` required for USA, BRA, CAN, MEX
...
const addressSchema = {
  ...
  province: {
    required: (formData) => {
      ...
      const multiTZCountries = ['USA', 'BRA', 'CAN', 'MEX'];
...
**Example:**
```http
GET /location-info/countries/USA/provinces
```
...
- [ ] Make `province` required for USA, BRA, CAN, MEX (optional but recommended)
- [ ] Display `timezone` from API response as read-only
- [ ] Update form validation to remove timezone validation
- [ ] Test address creation with single-TZ country (ARG)
- [ ] Test address creation with multi-TZ country (USA + province)
...
- Backend API: See [ADDRESSES_API_CLIENT.md](./ADDRESSES_API_CLIENT.md)
- Postman Tests: `docs/postman/TIMEZONE_DEDUCTION_TESTS.postman_collection.json`
