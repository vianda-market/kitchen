...
National holidays are currently checked during kitchen day selection (see `_find_next_available_kitchen_day_in_week()` in `plate_selection_validation.py`), which skips holidays when finding the next available day.
...
2. **Comprehensive Validation**: Plate selection must check both national and restaurant holidays
...
4. Integrate into plate selection flow after target date is calculated
...
- Integration: Integrated into plate selection flow
