...
National holidays are currently checked during kitchen day selection (see `_find_next_available_kitchen_day_in_week()` in `vianda_selection_validation.py`), which skips holidays when finding the next available day.
...
Restaurant-specific holiday validation will be added to the restaurant validation flow when the `restaurant_holidays` API is implemented.

**Key Principles**:
1. **No Duplication**: Restaurants cannot register holidays on dates that are already national holidays
2. **Comprehensive Validation**: Vianda selection must check both national and restaurant holidays
3. **National Holidays Take Precedence**: All orders are unavailable on national holidays, regardless of restaurant settings

**Proposed Implementation**:
1. Extend `validate_restaurant_status()` to `validate_restaurant()` with holiday parameters
2. Check both national and restaurant holidays for the target date
3. Provide user-friendly error messages distinguishing national vs. restaurant holidays
4. Integrate into vianda selection flow after target date is calculated

**Integration Point**:
```python
# Future implementation: validate_restaurant() function
...
✅ **Restaurant Status Validation**: Implemented
- Function: `validate_restaurant_status()`
- Location: `app/services/vianda_selection_validation.py`
- Checks: Restaurant status must be 'Active'

✅ **Restaurant Holiday Validation**: Implemented
- Function: `validate_restaurant()` (comprehensive validation)
- Location: `app/services/vianda_selection_validation.py`
- Checks: Restaurant status + national holidays + restaurant holidays
- Integration: Integrated into vianda selection flow

✅ **Restaurant Holidays API**: Implemented
- Routes: `app/routes/restaurant_holidays.py`
- Features: CRUD operations, bulk creation, national holiday validation
- Access Control: Suppliers (institution-scoped), Employees (global), Customers (blocked)
