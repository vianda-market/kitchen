# Vianda Pickup Pending Orders API

## Endpoint
```
GET /vianda-pickup/pending
```
...
interface PendingOrdersResponse {
  ...
  total_orders: number;             // Total count of all viandas
  orders: PlateOrderSummary[];      // Array of vianda orders (can be empty, 1, or multiple)
...
async function fetchPendingOrders(): Promise<PendingOrdersResponse | null> {
  const response = await fetch('/vianda-pickup/pending', {
    headers: {
...
3. **Orders array**: The `orders` field is an array that can contain:
   ...
   - **1 item**: Single vianda order
   - **Multiple items**: Multiple different viandas
...
- `POST /vianda-pickup/scan-qr`: Scan QR code to mark arrival
- `DELETE /vianda-pickup/{pickup_id}`: Delete a pickup record
