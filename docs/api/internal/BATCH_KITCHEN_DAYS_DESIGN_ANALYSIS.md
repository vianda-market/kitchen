...
When a user wants to assign a vianda to multiple kitchen days (e.g., Monday, Tuesday, Wednesday), we need to decide on the best approach:

1. **Client-side queuing**: Client makes multiple individual `POST /vianda-kitchen-days/` calls
2. **Batch endpoint**: Single `POST /vianda-kitchen-days/batch` call with array of days

## Use Case Analysis

### Typical Scenarios

1. **Initial vianda setup**: Assign 5 days (Monday-Friday) - **Most common**
...
The legacy `POST /vianda-selections/kitchen-days/{vianda_id}` endpoint:
- **Hard deletes** ALL existing kitchen days for the vianda
...
for (const day of days) {
  await fetch('/vianda-kitchen-days/', {
    method: 'POST',
    body: JSON.stringify({ vianda_id, kitchen_day: day })
  });
}
```
...
```python
# Backend
@router.post("/batch", response_model=List[ViandaKitchenDayResponseSchema])
def create_vianda_kitchen_days_batch(
    payload: ViandaKitchenDayBatchCreateSchema,
    current_user: dict = Depends(get_current_user),
    db: psycopg2.extensions.connection = Depends(get_db)
):
    """
    Create multiple kitchen day assignments for a vianda in a single atomic operation.
    
    This endpoint is optimized for the common use case of assigning multiple days
    to a vianda at once (e.g., Monday-Friday).
    """
...
**Schema**:
```python
class ViandaKitchenDayBatchCreateSchema(BaseModel):
    vianda_id: UUID
...
```typescript
// Simple client code
const result = await fetch('/vianda-kitchen-days/batch', {
  method: 'POST',
  body: JSON.stringify({
    vianda_id: plateId,
...
def create_vianda_kitchen_day(
    payload: Union[ViandaKitchenDayCreateSchema, ViandaKitchenDayBatchCreateSchema],
    ...
):
...
**Endpoint**: `POST /vianda-kitchen-days/batch`
...
**Response Schema**:
```python
class ViandaKitchenDayBatchResponseSchema(BaseModel):
    vianda_id: UUID
    created: List[ViandaKitchenDayResponseSchema]
...
- If vianda doesn't belong to institution → 403 Forbidden (no days created)
...
- Add `POST /vianda-kitchen-days/batch` endpoint
- Keep individual `POST /vianda-kitchen-days/` for single-day assignments
...
- Delete `POST /vianda-selections/kitchen-days/{vianda_id}` after clients migrated
