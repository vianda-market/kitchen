...
- **Admin** (`'Admin'`): Standard administrator with Employee role_type
- **Super Admin** (`'Super Admin'`): Super administrator with Employee role_type and special approval permissions
- **Management** (`'Management'`): Management role (varies by role_type)
- **Operator** (`'Operator'`): Operator role (varies by role_type)
...
## 4. Fintech Link API (`/fintech-link/`)
...
**Notes**:
- **Customers**: Can GET, PUT, DELETE only their own user record (scoped by `user_id`)
- **Suppliers**: Can access users within their institution (institution scoping)
- **Employees**: Have global access (can see all users)
