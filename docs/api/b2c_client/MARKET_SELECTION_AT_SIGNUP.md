...
1. Call **`GET /api/v1/leads/markets`** (no auth) to get the list of countries. The response returns **`country_code`** and **`country_name`** only (no `market_id`).
...
- **Refresh when entering signup:** Call GET /leads/markets when the user opens the signup screen (or use a short-TTL cache). Populate the dropdown from that response.
