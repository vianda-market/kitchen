...
| 3 | **Create Credit Currency**: On submit, send only `currency_name` (selected value) and `credit_value`. Do **not** send `currency_code`. |
...
When creating a new credit currency, the client sends only **currency name** and **credit_value**. The backend resolves `currency_code` from the supported list and stores it.
...
| `credit_value` | number   | Yes      | Must be > 0. |
...
201 Created. Response body is the created credit currency (includes `credit_currency_id`, `currency_name`, `currency_code`, `credit_value`, etc.). Use `credit_currency_id` when creating a market or linking to other resources.
...
- **Previous behavior**: `POST /api/v1/credit-currencies/` accepted `currency_name`, `currency_code`, and `credit_value`.
...
2. When creating a credit currency, send only `currency_name` (and `credit_value`). Remove any code that sends `currency_code` in the create payload.
...
| Create body | `currency_name` (required), `credit_value` (required). Do **not** send `currency_code`. |
| Auth | Employee only (Bearer token) |
