...
- `holiday_date DATE` - Specific date
- `is_recurring BOOLEAN` - Recurring holiday flag
- `recurring_month INTEGER` - For recurring holidays
- `recurring_day INTEGER` - For recurring holidays

**`restaurant_holidays`**:
- `restaurant_id UUID` - Foreign key to restaurant
- `country VARCHAR(100)` - Country name (not code)
- `holiday_date DATE` - Specific date
- `is_recurring BOOLEAN` - Recurring holiday flag
...
ON restaurant_holidays(restaurant_id, recurring_month_day) 
...
1. **Country Code Conversion**: How to convert `restaurant_holidays.country` (VARCHAR(100), name) to `national_holidays.country_code` (VARCHAR(3), ISO code)?
   - **Option A**: Add `country_code` to `restaurant_holidays` table
   - **Option B**: Lookup country code from `address_info` via `restaurant_info`
   - **Option C**: Create mapping table or function
   - **Recommendation**: Option B (lookup via restaurant → address)
