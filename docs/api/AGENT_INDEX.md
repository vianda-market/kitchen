...
**Array-filter wire format (multi-select):** Use repeated query params — `?cuisine=italian&cuisine=japanese` — never comma-joined. See `filters.md` §"multi-select (`in` op)" for the canonical rule and the FastAPI parsing rationale.
