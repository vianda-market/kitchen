...
- **country_code** is always **ISO 3166-1 alpha-3** (e.g. ARG, USA) for persistence. The suggest endpoint accepts optional `country` in query (alpha-2 or alpha-3) to restrict results.
...
- **Mapping**: [app/services/address_autocomplete_mapping.py](app/services/address_autocomplete_mapping.py) – Google address components to backend schema (route → street_type + street_name, country short → alpha-3, etc.).
