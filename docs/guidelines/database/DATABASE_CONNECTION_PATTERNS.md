...
The `db` connection is provided by FastAPI's `get_db` dependency (`app/dependencies/database.py`). It obtains a connection from the connection pool (`app/utils/db_pool.py`), is request-scoped, and is automatically returned to the pool when the request completes. Pass this connection through to all database operations within the request.
...
def db_delete(table: str, where: dict, connection=None, soft: bool = False, ...)
...
def db_batch_delete(table: str, where_list: List[dict], connection=None, soft: bool = False) -> int
```
...
For multi-operation atomic transactions, pass `commit=False` to `db_insert` and `db_update` so they do not commit immediately. Perform all operations, then commit the connection once at the end.
...
def create_entity(
    ...
    def _create_entity_with_validation():
        entity_data = entity.model_dump()  # Pydantic v2: use model_dump(), not dict()
...
`handle_business_operation` (`app/services/error_handling.py`) wraps the business logic with consistent error handling, logs success/failure, and re-raises HTTPExceptions.
