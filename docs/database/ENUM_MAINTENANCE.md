...
After the enum migration is complete, **`app/config/`** becomes the **single source of truth** for all enum values in the application. All enum classes are defined here and exported through `app/config/__init__.py`.
...
After the migration is complete, you **do NOT need to update**:
...
1. **Update Python enum**: `app/config/status.py`
...
1. **Update Python enums**: `app/config/role_types.py` and/or `app/config/role_names.py`
...
4. **Update `get_valid_for_role_type()`**: `app/config/role_names.py` - add new combination
...
**Fix**: 
- Check `app/config/{enum_name}.py` - value must exist in enum class
- Import and use enum: `from app.config.kitchen_days import KitchenDay`
