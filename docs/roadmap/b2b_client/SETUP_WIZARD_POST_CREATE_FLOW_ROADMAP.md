...
1. Creates a **primary entity** (e.g. restaurant, plate, institution).
2. Guides the user through **dependent configuration** steps (e.g. add plates, assign kitchen days, create QR code).
3. Optionally **activates** or **finalizes** the entity when all requirements are met.

### Key Characteristics

- **Linear or branching steps**: Each step may depend on the previous (e.g. "Add plates" requires restaurant_id from step 1).
...
|-----------|-------------|
| Step indicator | "Step 1 of 4" or breadcrumb (e.g. Restaurant → Plates → Kitchen Days → QR → Activate) |
...
**Source**: [RESTAURANT_STATUS_AND_PLATE_KITCHEN_DAYS.md](../backend/shared_client/RESTAURANT_STATUS_AND_PLATE_KITCHEN_DAYS.md)

**Current flow**: Create restaurant → Navigate to Plates → Create plates → Navigate to Kitchen Days → Assign days → Navigate to QR Codes → Create QR → Edit restaurant → Set status Active.

**Proposed wizard steps**:
1. Create restaurant (institution entity, address, name, cuisine).
2. Add plates (product, price, credit, delivery time) — can add multiple or skip.
3. Assign kitchen days for each plate (bulk Mon–Fri) — can use bulk API.
4. Create QR code(s) — at least one required for activation.
5. Activate restaurant (PUT status Active) — backend validates plates + kitchen days + QR.

**Backend coordination**: Activation requires plates with active plate_kitchen_days and at least one active QR code. No new API needed; wizard orchestrates existing endpoints.

---

### 2. Plate Setup

**Source**: [PLATE_API_CLIENT.md](../backend/shared_client/PLATE_API_CLIENT.md)

**Current flow**: Create plate → Navigate to Kitchen Days → Add one day at a time (or use bulk once UI supports it).

**Proposed wizard steps**:
1. Create plate (product_id, restaurant_id, price, credit, delivery_time_minutes).
2. Assign kitchen days — multi-select (Mon–Fri); bulk `POST /api/v1/plate-kitchen-days/` with `kitchen_days: [...]`.

**Backend coordination**: Bulk kitchen days API exists. Post-create modal or inline step on Plates page could offer "Assign kitchen days?" with plate pre-selected. See the Bulk Plate Kitchen Days UI plan for current implementation.
...
### 6. Product → Plate Setup

**Current flow**: Create product → Navigate to Plates → Create plate(s) linking product to restaurant(s).

**Proposed wizard steps**:
1. Create product (institution_id, name, ingredients, dietary).
2. Create plate(s) — for each restaurant, set price, credit, delivery_time_minutes. Optionally assign kitchen days in same flow.

**Backend coordination**: Plates require product_id and restaurant_id. Bulk plate create (multiple restaurants at once) could reduce round-trips. Not implemented today.
...
| Compound endpoint (e.g. `POST /restaurants/with-plates-and-qr`) | Single transaction; atomic | New endpoints; schema complexity |
| Optional nested body (e.g. `restaurant` + `plates[]` in one POST) | Flexible; one call | Validation complexity; partial success? |
...
|----------|----------|-----------|
| 1 | Plate setup (post-create kitchen days) | Bulk API exists; small scope |
...
| 6 | Product → Plate | Medium complexity |
...
4. **Iterate**: Start with Plate post-create (kitchen days) as pilot; expand to Restaurant wizard.
