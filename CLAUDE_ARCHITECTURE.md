...
- **Enhanced daily-orders:** `GET /api/v1/restaurant-staff/daily-orders` includes per-order: `vianda_pickup_id`, `expected_completion_time`, `completion_time`, `countdown_seconds`, `extensions_used`, `was_collected`, `confirmation_code`, `pickup_type`, `is_no_show`. Per-restaurant: `pickup_window_start/end`, `require_kiosk_code_verification`, `live_locked_count`, `reservations_by_vianda` (each entry: `vianda_id`, `vianda_name`, `count`, `completed_count`). Response: `server_time` for timer sync. Privacy: initials only (M.G.). Optional filter params: `status` (repeatable, native list, SQL-applied) and `is_no_show` (bool, service-applied). Schemas: `DailyOrdersResponseSchema` → `RestaurantDailyOrdersSchema` → `ReservationByViandaSchema` + `DailyOrderItemSchema` + `OrderSummarySchema`.
...
- **Dev wrapper — `app/db/build_dev_db.sh`:** One-command dev-rebuild driver above the demo loader. Calls `build_kitchen_db.sh` (schema + reference + dev fixtures) then `load_demo_data.sh` (demo_baseline.sql + 900 Newman seed). Pass `--target=local|gcp-dev` (forwarded to the loader). Used by the `db-rebuild.yml` GH Actions workflow (renamed from `db-reset-dev.yml`) with `seed=demo` (`--target=gcp-dev`) and by devs locally (default `--target=local`). `build_kitchen_db.sh` itself stays the primitive — staging/prod composition (when those envs exist) will compose from it without the demo layer.
