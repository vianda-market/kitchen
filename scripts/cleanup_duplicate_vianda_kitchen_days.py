#!/usr/bin/env python3
"""
Cleanup script: archive duplicate vianda kitchen day rows, keeping one canonical
row per (vianda_id, kitchen_day) combination.

Problem context (issue #190):
    Postman collections were POSTing to /api/v1/vianda-kitchen-days on every run
    without cleanup, accumulating many copies of functionally identical scheduling
    rows (e.g. multiple active rows for the same vianda on Monday).
...
What this script does:
    For each (vianda_id, kitchen_day) combination, finds all non-archived rows
    ...
    PUT /vianda-kitchen-days/by-key upsert endpoint and do not need cleanup.
...
Usage:
    ...
    python scripts/cleanup_duplicate_vianda_kitchen_days.py --dry-run

    # Live run:
    python scripts/cleanup_duplicate_vianda_kitchen_days.py
...
def find_duplicate_groups(cursor) -> list[dict]:
    """
    Return groups of duplicate vianda kitchen days (same vianda_id + kitchen_day,
    ...
    Each group is a dict:
        {
            ...
            "keep_pkd_id": str,              # vianda_kitchen_day_id to retain
            "archive_pkd_ids": [str, ...],   # vianda_kitchen_day_ids to soft-delete
        }
    """
    cursor.execute(
        """
        SELECT
            vianda_id::text,
            kitchen_day,
            array_agg(vianda_kitchen_day_id::text ORDER BY created_date ASC) AS pkd_ids
        FROM ops.vianda_kitchen_days
        WHERE is_archived = FALSE
          AND canonical_key IS NULL
        GROUP BY vianda_id, kitchen_day
        HAVING COUNT(*) > 1
        ORDER BY vianda_id, kitchen_day
        """
    )
    rows = cursor.fetchall()
    groups = []
    for vianda_id, kitchen_day, pkd_ids in rows:
        groups.append(
            {
                "vianda_id": vianda_id,
...
def archive_vianda_kitchen_days(cursor, pkd_ids: list[str], dry_run: bool) -> int:
    """Soft-delete the given vianda_kitchen_day_ids.  Returns the count archived."""
    if not pkd_ids:
        return 0
    if dry_run:
        return len(pkd_ids)
    cursor.execute(
        ...
        SET is_archived = TRUE,
            ...
            modified_by = %s::uuid
        WHERE vianda_kitchen_day_id = ANY(%s::uuid[])
...
def run(dry_run: bool = False) -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not groups:
                print("No duplicate vianda kitchen days found — database is clean.")
                return
            ...
            for group in groups:
                ...
                print(
                    ...
                    f"vianda_id={group['vianda_id']} "
                    f"kitchen_day={group['kitchen_day']} "
                    f"(keeping {group['keep_pkd_id']})"
                )
                archived = archive_vianda_kitchen_days(cur, group["archive_pkd_ids"], dry_run)
                total_archived += archived

            if dry_run:
                print(f"\nDry-run complete: {total_archived} vianda kitchen day(s) would be archived.")
                conn.rollback()
            else:
                conn.commit()
                print(f"\nDone: {total_archived} vianda kitchen day(s) archived.")
    finally:
        conn.close()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Archive duplicate vianda kitchen day rows (keep oldest per vianda_id+kitchen_day)"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
