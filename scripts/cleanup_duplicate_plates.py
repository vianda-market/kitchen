#!/usr/bin/env python3
"""
Cleanup script: archive duplicate plate rows, keeping one canonical row per restaurant+product.

Problem context (issue #166):
    Postman collections were POSTing to /api/v1/plates on every run without
    cleanup, accumulating many copies of functionally identical plates
    (e.g. dozens of rows for the same product at the same restaurant).
    These clutter the admin UI and the supplier's plate list.

What this script does:
    For each restaurant, finds all plate rows that share the same
    ...
    Plates with a canonical_key are left untouched — they are managed by the
...
def find_duplicate_groups(cursor) -> list[dict]:
    """
    Return groups of duplicate plates (same restaurant_id + product_id, no canonical_key).

    Each group is a dict:
        {
            "restaurant_id": str,
            "product_id": str,
            "plate_ids": [str, ...],   # oldest first
            "keep_plate_id": str,      # plate_id to retain
            "archive_plate_ids": [str, ...],  # plate_ids to soft-delete
        }
    """
    cursor.execute(
        """
        SELECT
            restaurant_id::text,
            product_id::text,
            array_agg(plate_id::text ORDER BY created_date ASC) AS plate_ids
        FROM ops.plate_info
    ...
    groups = []
    for restaurant_id, product_id, plate_ids in rows:
        groups.append(
            {
                "restaurant_id": restaurant_id,
                "product_id": product_id,
                "plate_ids": plate_ids,
                "keep_plate_id": plate_ids[0],
                "archive_plate_ids": plate_ids[1:],
            }
        )
    return groups


def archive_plates(cursor, plate_ids: list[str], dry_run: bool) -> int:
    """Soft-delete the given plate_ids.  Returns the count archived."""
    if not plate_ids:
        return 0
    if dry_run:
    ...
    cursor.execute(
        """
        UPDATE ops.plate_info
        SET is_archived = TRUE,
            modified_date = CURRENT_TIMESTAMP,
            modified_by = %s::uuid
        WHERE plate_id = ANY(%s::uuid[])
          AND is_archived = FALSE
        """,
        (SYSTEM_USER_ID, plate_ids),
    )
...
def run(dry_run: bool = False) -> None:
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if not groups:
                print("No duplicate plates found — database is clean.")
                return

            total_archived = 0
            for group in groups:
                n = len(group["archive_plate_ids"])
                ...
                print(
                    ...
                    f"(keeping {group['keep_plate_id']})"
                )
                archived = archive_plates(cur, group["archive_plate_ids"], dry_run)
                total_archived += archived

            if dry_run:
                print(f"\nDry-run complete: {total_archived} plate(s) would be archived.")
                conn.rollback()
            else:
                conn.commit()
                print(f"\nDone: {total_archived} plate(s) archived.")
    finally:
        conn.close()


def main() -> None:
    parser = argparse.ArgumentParser(description="Archive duplicate plate rows (keep oldest per restaurant+product)")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=False,
