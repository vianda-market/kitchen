...
- Never assume `gh pr update-branch` leaves the PR mergeable. After a rebase, the `Deploy Kitchen Backend` workflow on the new SHA is often concurrency-cancelled (its prior in-flight run on the old SHA wins the group), so the required `Require deploy:dev label` check never posts and the PR sits at `mergeStateStatus: BLOCKED` with zero failing or pending checks. Recovery: `gh run list --repo vianda-market/kitchen --branch <branch> --workflow="Deploy Kitchen Backend" --limit 1 --json conclusion,databaseId` — if conclusion is `cancelled` and the head SHA matches the PR's, `gh run rerun <databaseId> --repo vianda-market/kitchen`. After a batch rebase, audit every stuck PR for this — it bit four PRs in a row on 2026-05-13.
- Never run `pulumi up dev` in `infra-kitchen-gcp` without immediately re-dispatching `Deploy Kitchen Backend` against `main`. Pulumi creates a new Cloud Run revision using state's bootstrap `image_tag` whenever it updates the service template (env var, scaling, ingress, etc.); `ignore_changes` filters the diff in `pulumi preview` but doesn't stop the new revision from being created with the stale template image. Symptom: dev API was healthy until the infra deploy, then endpoints start returning 500 (often `POST /api/v1/addresses` first) — confirmed 2026-05-14 on infra#160 (`payment_provider=stripe` flip). Recovery: `gh workflow run "Deploy Kitchen Backend" --repo vianda-market/kitchen --ref main -f reason="recover dev image after infra pulumi up"`. Full rationale: [`infra-kitchen-gcp/CLAUDE.md` → "Cloud Run image ownership"](https://github.com/vianda-market/infra-kitchen-gcp/blob/main/CLAUDE.md#cloud-run-image-ownership-read-before-touching-backendpy-or-pulumistackyaml).
