...
3. **Database changes**: Do not write or run database migrations. Assume the database will be **torn down and rebuilt** for schema changes. Do not add migration scripts unless explicitly requested. Primary keys use **UUID7** for new rows (see [docs/database/UUID7_MIGRATION_PLAN.md](docs/database/UUID7_MIGRATION_PLAN.md)); for PostgreSQL &lt; 18, run `app/db/uuid7_function.sql` before schema. When changing schema, sync **all** layers in order: `schema.sql` → `trigger.sql` → `seed.sql` → DTOs (`app/dto/models.py`) → Pydantic schemas (if exposed via API). DTOs are often forgotten—missing fields there break inserts.
