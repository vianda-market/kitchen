...
from app.utils.db_pool import get_db_connection, close_db_connection
...
def _is_enum_registered(connection, enum_name: str) -> bool:
    """
...
def _prepare_value_for_db(value: Any, table: str, column: str, connection=None) -> Any:
    ...
    enum_mappings = {
        ...
        ('credit_currency_info', 'status'): 'status_enum',
...
def db_batch_insert(table: str, data_list: List[dict], connection=None) -> List:
    """
...
def _build_update_sql(table: str, data: dict, where: dict, connection=None) -> Tuple[str, tuple]:
    """
...
def db_update(table: str, data: dict, where: dict, connection=None, *, commit: bool = True):
    ...
    try:
        ...
        connection.rollback()
        raise handle_database_exception(e, f"update {table}")
    finally:
        if new_connection:
            close_db_connection(connection)
...
def db_batch_update(
    ...
    Args:
        table: Table name
        updates: Either a single dict (Pattern 1) or list of dicts (Pattern 2)
        where_list: List of WHERE clause dicts (required for Pattern 1, ignored for Pattern 2)
        connection: Optional database connection (if None, creates and manages connection)
    ...
    Raises:
        ValueError: If updates is empty, invalid, or pattern is ambiguous
        Exception: If any update fails, all operations are rolled back
    
    Example:
        # Pattern 1: Same update for multiple records
        updates = {"status": "archived"}
        where_list = [{"id": "uuid1"}, {"id": "uuid2"}]
    ...
    is_pattern_1 = isinstance(updates, dict)
    is_pattern_2 = isinstance(updates, list)
    ...
    if is_pattern_1:
        ...
        update_list = [updates] * len(where_list)  # Same update for all
        where_clause_list = where_list
        ...
        for i, update_dict in enumerate(updates):
            ...
            # For Pattern 2, we need to extract id from each update dict
            # Assume 'id' is the primary key (could be made configurable)
            if "id" not in update_dict:
                raise ValueError(f"updates[{i}] must contain 'id' field for Pattern 2")
        
        # Extract WHERE clauses from updates (using 'id' field)
        update_list = []
        where_clause_list = []
        for update_dict in updates:
            # Separate id from other fields
            update_data = {k: v for k, v in update_dict.items() if k != "id"}
            where_clause = {"id": update_dict["id"]}
    ...
    try:
        ...
        for i, (update_data, where_clause) in enumerate(zip(update_list, where_clause_list)):
        ...
        raise handle_database_exception(e, f"batch update {table}")
    finally:
        if new_connection:
            close_db_connection(connection)
...
def db_read(query: str, values: tuple = None, connection=None, fetch_one: bool = False):
    ...
    try:
        ...
        if raw_results is None:
            ...
            results = dict(zip(columns, raw_results)) if raw_results else None
        else:
            # Multiple records - return list of dictionaries
            results = [dict(zip(columns, row)) for row in raw_results] if raw_results else []
...
def db_delete(table: str, where: dict, connection=None, soft: bool = False, soft_update_fields: Optional[dict] = None):
    """
...
def db_batch_delete(table: str, where_list: List[dict], connection=None, soft: bool = False, soft_update_fields: Optional[dict] = None) -> int:
    """
    ...
    try:
        ...
        raise handle_database_exception(e, f"batch delete from {table}")
    finally:
        if new_connection:
            close_db_connection(connection)
