...
from app.services.crud_service import get_bills_by_restaurant_and_period, institution_bill_service
from app.services.entity_service import (
    get_bills_by_status,
    get_enriched_institution_bills,
    get_pending_bills_by_institution,
)
...
def get_institution_bill(
    bill_id: UUID, current_user: dict = Depends(get_current_user), db: psycopg2.extensions.connection = Depends(get_db)
):
    """Get a specific institution bill by ID"""
    return handle_get_by_id(institution_bill_service.get_by_id, bill_id, db, "institution bill")
...
def update_institution_bill(
    ...
    def _update_institution_bill():
        ...
        if payload.resolution is not None:
            update_data["resolution"] = (
                payload.resolution.value if hasattr(payload.resolution, "value") else payload.resolution
            )
    ...
    return handle_business_operation(
        _update_institution_bill, "institution bill update", "Institution bill updated successfully"
    )
...
def cancel_bill(
    ...
    def _cancel_bill():
        result = InstitutionBillingService.cancel_bill(bill_id=bill_id, user_id=current_user["user_id"], connection=db)
...
def generate_daily_bills(
    bill_date: date | None = Query(None, description="Bill date (default: today)"),
    country_code: str | None = Query(None, description="Country code (e.g. US); auto-detected if omitted"),
    current_user: dict = Depends(get_current_user),
    db: psycopg2.extensions.connection = Depends(get_db),
):
...
def get_billing_summary(
    ...
    def _get_billing_summary():
        summary = InstitutionBillingService.get_bill_summary_by_institution(
            institution_id, start_date, end_date, connection=db
        )
