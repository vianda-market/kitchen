...
def _check_unique_constraint(
    plate_id: UUID, kitchen_day: str, db: psycopg2.extensions.connection, exclude_id: UUID | None = None
) -> bool:
    """
...
def create_plate_kitchen_day(
    ...
    def create_operation(connection: psycopg2.extensions.connection):
        ...
        for inserted_id in inserted_ids:
            kitchen_day = plate_kitchen_days_service.get_by_id(UUID(inserted_id), connection, scope=scope)
            if kitchen_day:
...
def update_plate_kitchen_day(
    ...
    def update_operation(connection: psycopg2.extensions.connection):
        ...
        if wants_archive:
            ...
            ok = plate_kitchen_days_service.soft_delete(
