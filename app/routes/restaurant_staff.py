...
def get_restaurant_daily_orders(
    restaurant_id: Optional[UUID] = Query(None, description="Filter to specific restaurant"),
    order_date: Optional[date_type] = Query(None, description="Date to query (defaults to today)"),
    ...
    def _get_daily_orders():
        ...
        if current_user["role_type"] == "Supplier":
            ...
            if not result:
                raise HTTPException(
                    status_code=404, 
                    detail="Institution entity not found for user's institution"
                )
