...
async def get_pool_stats(
    ...
    def _get_pool_stats():
        ...
        return {"status": "error", "message": "Failed to retrieve pool statistics"}

    result = handle_business_operation(_get_pool_stats, "pool statistics retrieval")

    if not result:
        return {"status": "error", "message": "Error retrieving pool statistics"}
