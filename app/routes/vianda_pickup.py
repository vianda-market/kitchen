...
from app.schemas.consolidated_schemas import ViandaPickupEnrichedResponseSchema
from app.services.entity_service import get_enriched_vianda_pickups
...
router = APIRouter(prefix="/vianda-pickup", tags=["Vianda Pickup"], dependencies=[Depends(oauth2_scheme)])
...
class PlateOrderSummary(BaseModel):
    ...
    vianda_pickup_id: UUID | None = Field(None, description="Use for POST /vianda-pickup/{id}/complete")
...
class PendingOrdersResponse(BaseModel):
    ...
    total_orders: int  # Total count of all viandas
    total_vianda_count: int | None = Field(None, description="Same as total_orders; viandas the assigned user picks up")
    vianda_pickup_ids: list[UUID] | None = Field(None, description="IDs for POST /vianda-pickup/{id}/complete")
...
def get_pending_order(
    ...
    def _get_pending_order():
        result = vianda_pickup_service.get_pending_orders_with_company_matching(current_user["user_id"], db)
        ...
        orders = [
            PlateOrderSummary(
                vianda_name=order["vianda_name"],
                order_count=order["order_count"],
                delivery_time_minutes=order["delivery_time_minutes"],
                vianda_pickup_id=order.get("vianda_pickup_id"),
            )
        ...
        return PendingOrdersResponse(
            ...
            total_vianda_count=result.get("total_vianda_count", result["total_orders"]),
            vianda_pickup_ids=result.get("vianda_pickup_ids"),
            orders=orders,
...
@router.get("/enriched", response_model=list[ViandaPickupEnrichedResponseSchema])
def get_enriched_vianda_pickups_endpoint(  # noqa: PLR0913 — declarative FastAPI Query params, not algorithmic args
    response: Response,
    ...
    vianda_id: list[UUID] | None = Query(None, description="Filter by vianda ID(s) (multi-select)"),
    ...
    completion_time_from, completion_time_to, was_collected, credit_from, credit_to, restaurant_id, vianda_id.
    Use completed_only=true for the customer order history page (pickups they have collected).

    Scoping:
    - Internal: See all vianda pickups
    - Suppliers: See vianda pickups for restaurants in their institution
    - Customers: See only their own vianda pickups"""
    from app.security.entity_scoping import ENTITY_VIANDA_PICKUP_LIVE, EntityScopingService

    def _get_enriched_pickups():
        scope = EntityScopingService.get_scope_for_entity(ENTITY_VIANDA_PICKUP_LIVE, current_user)
        user_id = None
        ...
        if current_user.get("role_type") == "customer":
            try:
                ...
                raise envelope_exception(ErrorCode.VIANDA_PICKUP_INVALID_USER_ID, status=400, locale=locale) from None
        ...
        try:
            filter_conditions = build_filter_conditions(
                "pickups",
                {
                    ...
                    "vianda_id": [str(p) for p in vianda_id] if vianda_id else None,
                },
            )
        except ValueError as exc:
            log_warning(f"Invalid filter on /vianda-pickups/enriched: {exc}")
            raise envelope_exception(ErrorCode.VIANDA_PICKUP_INVALID_FILTER, status=400, locale=locale) from None
        return get_enriched_vianda_pickups(
            db,
            scope=scope,
            user_id=user_id,
    ...
    result = handle_business_operation(_get_enriched_pickups, "enriched vianda pickup retrieval")
...
def scan_qr_code(
    ...
    def _scan_qr_code():
        result = vianda_pickup_service.scan_qr_code_by_id(request.qr_code_id, request.sig, current_user, db)
...
def hand_out_order(
    ...
    """Mark order as Handed Out (restaurant gave the vianda to customer). One-tap kiosk action.
    ...
    def _hand_out():
        ...
        if current_user["role_type"] not in ("supplier", "internal"):
            raise envelope_exception(ErrorCode.VIANDA_PICKUP_STAFF_ONLY, status=403, locale=locale)
...
def complete_order(
    ...
    - user_confirmed: User tapped "I received my vianda" (default)
    ...
    def _complete_order():
        ...
        return vianda_pickup_service.complete_order(pickup_id, current_user, db, completion_type=completion_type)

    return handle_business_operation(_complete_order, "order completion")


# DELETE /vianda-pickup/{pickup_id} – Delete (soft-delete) a vianda pickup record
@router.delete("/{pickup_id}", response_model=dict)
def delete_vianda_pickup(
    pickup_id: UUID,
    current_user: dict = Depends(get_current_user),
    db: psycopg2.extensions.connection = Depends(get_db),
):
    ...
    def _delete_vianda_pickup():
        return vianda_pickup_service.delete_pickup_record(pickup_id, current_user, db)

    return handle_business_operation(_delete_vianda_pickup, "vianda pickup deletion")


# POST /vianda-pickup/run-promotion – Manual trigger for kitchen-start promotion cron
...
def run_kitchen_start_promotion_manual(
    ...
    """
    Manually trigger the kitchen-start promotion: promote active vianda_selection_info
    rows to vianda_pickup_live (creates pickup records + restaurant transactions).
