...
from app.services.cron.archival_job import run_daily_archival, run_archival_validation, get_archival_dashboard
...
async def archive_specific_records(
    ...
    def _archive_specific_records():
        archived_count = ArchivalService.archive_records(
            entity_type, 
            record_ids, 
            current_user["user_id"]
        )
        
        log_info(f"Manual archival of {len(record_ids)} {entity_type} records by user {current_user['user_id']}: {archived_count} archived")
...
async def get_retention_policy(current_user: dict = Depends(get_current_user)):
    ...
    def _get_retention_policy():
        ...
        return {
            "status": "success",
            "policy": policy,
            "note": "Retention periods are in days"
        }
