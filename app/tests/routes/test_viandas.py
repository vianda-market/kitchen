"""
Tests for vianda upsert:
- PUT /viandas/by-key: insert, update, and idempotency.
- PUT /viandas/by-key: auth guard (non-employee returns 403).
"""
...
def _make_vianda_dto(*, canonical_key=None):
    """Build a minimal mock vianda DTO."""
    m = MagicMock()
    m.vianda_id = uuid4()
    m.product_id = uuid4()
...
def _valid_upsert_payload(*, canonical_key="TEST_RESTAURANT_VIANDA_BONDIOLA"):
...
class TestViandaUpsertByKey:
    """PUT /api/v1/viandas/by-key: insert, update, idempotency, auth."""

    @patch("app.services.crud_service.find_vianda_by_canonical_key")
    @patch.object(vianda_service, "create")
    def test_upsert_inserts_when_key_not_found(self, mock_create, mock_find, client_with_employee):
        """PUT /viandas/by-key with a new canonical_key inserts a new vianda and returns 200."""
        mock_find.return_value = None  # key does not exist yet
        created = _make_vianda_dto(canonical_key="TEST_RESTAURANT_VIANDA_BONDIOLA")
        mock_create.return_value = created

        payload = _valid_upsert_payload(canonical_key="TEST_RESTAURANT_VIANDA_BONDIOLA")
        resp = client_with_employee.put("/api/v1/viandas/by-key", json=payload)

        assert resp.status_code == 200
        data = resp.json()
        assert "vianda_id" in data
        mock_create.assert_called_once()

    @patch("app.services.crud_service.find_vianda_by_canonical_key")
    @patch.object(vianda_service, "update")
    def test_upsert_updates_when_key_exists(self, mock_update, mock_find, client_with_employee):
        """PUT /viandas/by-key with an existing canonical_key updates the vianda and returns 200."""
        existing = _make_vianda_dto(canonical_key="TEST_RESTAURANT_VIANDA_BONDIOLA")
        mock_find.return_value = existing
        updated = _make_vianda_dto(canonical_key="TEST_RESTAURANT_VIANDA_BONDIOLA")
        ...
        # canonical_key must not be passed to vianda_service.update (update strips it)
    ...
    def test_upsert_idempotent_same_payload_twice(self, mock_create, mock_find, client_with_employee):
        ...
        created = _make_vianda_dto(canonical_key="TEST_RESTAURANT_VIANDA_IDEMPOTENT")
        mock_create.return_value = created

        payload = _valid_upsert_payload(canonical_key="TEST_RESTAURANT_VIANDA_IDEMPOTENT")
        resp1 = client_with_employee.put("/api/v1/viandas/by-key", json=payload)
        assert resp1.status_code == 200

        # Second call: key now exists — should call update instead of create
        mock_find.return_value = created  # simulate the vianda now existing
        with patch.object(vianda_service, "update") as mock_update:
            mock_update.return_value = created
            resp2 = client_with_employee.put("/api/v1/viandas/by-key", json=payload)
            assert resp2.status_code == 200
            mock_update.assert_called_once()

    def test_upsert_requires_employee_auth(self):
        """PUT /viandas/by-key without employee auth returns 403."""
        ...
        try:
            with TestClient(app) as c:
                ...
                resp = c.put("/api/v1/viandas/by-key", json=payload)
    ...
    def test_upsert_rejects_supplier_role(self):
        """PUT /viandas/by-key with a supplier (non-internal) token returns 403.

        Mirrors exactly the role the Postman E2E collection uses during the
        Supplier Menu Setup phase.  The Postman step 'Upsert Canonical Vianda'
