...
class TestMessagingPreferencesService:
    ...
    def test_get_returns_default_row_with_new_fields_when_missing(self, mock_insert, mock_read):
        """When no row exists, default row includes coworkers_can_see_my_orders and can_participate_in_plate_pickups."""
        user_id = uuid4()
        ...
        assert result.can_participate_in_plate_pickups is True
        insert_call = mock_insert.call_args
        assert "coworkers_can_see_my_orders" in insert_call[0][1]
        assert "can_participate_in_plate_pickups" in insert_call[0][1]
        assert insert_call[0][1]["coworkers_can_see_my_orders"] is True
        assert insert_call[0][1]["can_participate_in_plate_pickups"] is True
    ...
    def test_update_can_participate_false_cascades_to_coworkers_and_notify(self, mock_update, mock_get):
        """When can_participate_in_plate_pickups is set to False, cascade sets coworkers_can_see_my_orders and notify_coworker_pickup_alert to False."""
        user_id = uuid4()
        mock_get.return_value = MagicMock(
            coworkers_can_see_my_orders=True,
            can_participate_in_plate_pickups=False,
        ...
        assert call_kwargs["can_participate_in_plate_pickups"] is False
    ...
    def test_update_new_fields_preserves_marketing_prefs_on_cascade(self, mock_update, mock_get):
        """Cascade when can_participate_in_plate_pickups=False does not change marketing or plate_ready prefs."""
        user_id = uuid4()
        mock_get.return_value = MagicMock(
            notify_promotions_push=True,
            notify_promotions_email=True,
        ...
        # Marketing and plate_ready not in update (not sent, not cascaded)
        assert "notify_promotions_push" not in call_kwargs
        assert "notify_promotions_email" not in call_kwargs
        assert "notify_plate_readiness_alert" not in call_kwargs
