...
def enriched_service_no_institution():
    ...
    return EnrichedService(
        ...
        table_alias="tt",
        id_column="test_id",
        schema_class=MockEnrichedSchemaOptional
    )
...
class TestBuildWhereClause:
    ...
    def test_build_where_clause_global_scope(self, enriched_service):
        """Test WHERE clause with global scope (Employee Admin)"""
        scope = InstitutionScope(
            institution_id=str(uuid4()),
            role_type="Employee",
...
class TestGetEnriched:
    ...
    def test_get_enriched_success(self, mock_db_read, enriched_service, mock_db):
        ...
        row1 = MagicMock()
        row1.__iter__ = lambda self: iter([("id", str(uuid4())), ("name", "Test 1"), ("institution_id", str(uuid4())), ("is_archived", False)])
        row1.items = lambda: [("id", str(uuid4())), ("name", "Test 1"), ("institution_id", str(uuid4())), ("is_archived", False)]
        row1.keys = lambda: ["id", "name", "institution_id", "is_archived"]
        row1.values = lambda: [str(uuid4()), "Test 1", str(uuid4()), False]
        
        row2 = MagicMock()
        row2.__iter__ = lambda self: iter([("id", str(uuid4())), ("name", "Test 2"), ("institution_id", str(uuid4())), ("is_archived", False)])
        row2.items = lambda: [("id", str(uuid4())), ("name", "Test 2"), ("institution_id", str(uuid4())), ("is_archived", False)]
    ...
    def test_get_enriched_database_error(self, mock_db_read, enriched_service, mock_db):
        ...
        assert "Failed to execute enriched query" in exc_info.value.detail or "Failed to get enriched" in exc_info.value.detail
...
class TestGetEnrichedById:
    ...
    def test_get_enriched_by_id_with_scope(self, mock_db_read, enriched_service, mock_db):
        ...
        call_args = mock_db_read.call_args
        query = call_args[0][0]
        assert f"tt.test_id = %s" in query
        assert "tt.institution_id = %s::uuid" in query
...
class TestExecuteQuery:
    ...
    def test_execute_query_success(self, mock_db_read, enriched_service, mock_db):
        ...
        results = enriched_service._execute_query(
            "SELECT * FROM test",
            None,
            mock_db,
            fetch_one=False
    ...
    def test_execute_query_error(self, mock_db_read, enriched_service, mock_db):
        ...
        with pytest.raises(HTTPException) as exc_info:
            enriched_service._execute_query(
                "SELECT * FROM test",
                None,
                mock_db,
                fetch_one=False
