...
class TestClientBillService:
    ...
    def test_create_client_bill_resolves_currency_code(self, sample_bill_data, sample_current_user, mock_db):
        """Test that client bill creation resolves currency code from credit currency."""
        # Arrange
        mock_currency = Mock()
        mock_currency.currency_code = "USD"
        
        mock_bill = Mock()
        mock_bill.bill_id = uuid4()
        
        with patch('app.services.client_bill_service.client_bill_service') as mock_bill_service, \
             patch('app.services.client_bill_service.resolve_currency_code') as mock_resolve_currency:
            ...
            mock_bill_service.create.return_value = mock_bill
            
            # Act
            result = client_bill_business_service.create_client_bill(
                sample_bill_data, sample_current_user, mock_db
            )
            
            # Assert
            mock_resolve_currency.assert_called_once_with(sample_bill_data, mock_db)
            assert sample_bill_data["currency_code"] == "USD"
            assert sample_bill_data["modified_by"] == sample_current_user["user_id"]

    def test_create_client_bill_validates_bill_data(self, sample_current_user, mock_db):
        """Test that client bill creation validates required bill data."""
        # Arrange
        invalid_bill_data = {
        ...
        with pytest.raises(HTTPException) as exc_info:
            client_bill_business_service.create_client_bill(
                invalid_bill_data, sample_current_user, mock_db
            )
        
        assert exc_info.value.status_code == status.HTTP_400_BAD_REQUEST
        assert "Amount must be a positive number" in str(exc_info.value.detail)

    def test_create_client_bill_applies_creation_rules(self, sample_bill_data, sample_current_user, mock_db):
        """Test that client bill creation applies business rules."""
        # Arrange
        mock_currency = Mock()
        mock_currency.currency_code = "USD"
        ...
        with patch('app.services.client_bill_service.client_bill_service') as mock_bill_service, \
            ...
            # Assert
            assert sample_bill_data["status"] == Status.PENDING
            assert sample_bill_data["is_archived"] is False
            assert "created_date" in sample_bill_data
