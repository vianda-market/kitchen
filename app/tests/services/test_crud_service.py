...
def direct_scoping_service():
    ...
    return CRUDService(
        table_name="test_table", dto_class=MockDirectScopingDTO, id_column="id", institution_column="institution_id"
    )
...
def join_scoping_service():
    ...
    return CRUDService(
        ...
        id_column="id",
        institution_join_path=[("INNER", "related_table", "rt", "test_table.foreign_key_id = rt.related_id")],
...
def supplier_scope():
    ...
    institution_id = uuid4()
    return InstitutionScope(institution_id=institution_id, role_type="supplier")
...
class TestBuildJoinQueryWithScope:
    ...
    def test_build_join_query_custom_order_by(self, multi_join_scoping_service, supplier_scope):
        ...
        query, params = multi_join_scoping_service._build_join_query_with_scope(
            scope=supplier_scope, include_archived=False, order_by="kitchen_day ASC"
        )
    ...
    def test_build_join_query_requires_join_path(self, direct_scoping_service, supplier_scope):
        ...
        with pytest.raises(ValueError, match="institution_join_path"):
            direct_scoping_service._build_join_query_with_scope(scope=supplier_scope, include_archived=False)
...
class TestValidateJoinBasedScope:
    ...
    @patch("app.utils.db.db_read")
    def test_validate_join_based_scope_not_found(
        self, mock_db_read, multi_join_scoping_service, mock_db, supplier_scope
    ):
    ...
    @patch("app.services.crud_service.db_read")
    def test_validate_join_based_scope_global_scope(
        self, mock_db_read, multi_join_scoping_service, mock_db, global_scope
    ):
...
class TestCreateWithJoinScoping:
    ...
    @patch("app.utils.db.db_read")
    @patch("app.services.crud_service.db_read")
    def test_create_with_foreign_key_validation(
        self, mock_crud_db_read, mock_utils_db_read, mock_db_insert, multi_join_scoping_service, mock_db, supplier_scope
    ):
    ...
    @patch("app.utils.db.db_read")
    def test_create_fails_foreign_key_validation(
        self, mock_db_read, multi_join_scoping_service, mock_db, supplier_scope
    ):
...
class TestUpdateWithJoinScoping:
    ...
    @patch("app.utils.db.db_read")
    @patch("app.services.crud_service.db_read")
    def test_update_with_foreign_key_validation(
        self, mock_crud_db_read, mock_utils_db_read, mock_db_update, multi_join_scoping_service, mock_db, supplier_scope
    ):
    ...
    @patch("app.utils.db.db_read")
    @patch("app.services.crud_service.db_read")
    def test_update_fails_foreign_key_validation(
        self, mock_crud_db_read, mock_utils_db_read, multi_join_scoping_service, mock_db, supplier_scope
    ):
...
class TestSoftDeleteWithJoinScoping:
    ...
    @patch("app.services.crud_service.db_update")
    @patch("app.services.crud_service.db_read")
    def test_soft_delete_with_join_scoping(
        self, mock_db_read, mock_db_update, multi_join_scoping_service, mock_db, supplier_scope
    ):
