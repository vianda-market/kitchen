...
class TestMapboxStaticGateway:
    ...
    def test_dev_mode_returns_png_bytes(self, mock_settings):
    ...
    @patch("app.gateways.base_gateway.get_settings")
    def test_url_length_within_limit(self, mock_settings):
        """30 markers should produce a URL under 8192 chars."""
        mock_settings.return_value = Mock(DEV_MODE=True)
        gw = MapboxStaticGateway()
        markers = [
            {"name": f"Restaurant {i}", "lat": -34.59 + i * 0.001, "lng": -58.40 + i * 0.001}
            for i in range(30)
        ]
    ...
    @patch("app.gateways.base_gateway.get_settings")
    def test_empty_markers_produces_map_without_overlays(self, mock_settings):
        mock_settings.return_value = Mock(DEV_MODE=True)
        gw = MapboxStaticGateway()
        result = gw.generate_static_map(
            style_id="mapbox/light-v11",
