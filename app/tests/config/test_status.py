"""
Unit tests for STATUS_CONTEXTS plate and plan entries.

Verifies that:
1. get_by_context("plate") returns only catalog-visibility values (active, inactive).
...
class TestStatusContextPlate(unittest.TestCase):
    """Tests for STATUS_CONTEXTS["plate"]."""

    def setUp(self) -> None:
        self.values = Status.get_by_context("plate")

    def test_plate_context_returns_list(self) -> None:
        self.assertIsInstance(self.values, list)

    def test_plate_context_has_active(self) -> None:
        self.assertIn("active", self.values)

    def test_plate_context_has_inactive(self) -> None:
        self.assertIn("inactive", self.values)

    def test_plate_context_exact_members(self) -> None:
        """plate context must contain exactly active and inactive — no more, no less."""
        self.assertEqual(set(self.values), {"active", "inactive"})

    def test_plate_context_excludes_processed(self) -> None:
        """processed is billing-only and must not appear in plate context."""
        self.assertNotIn("processed", self.values)

    def test_plate_context_excludes_pickup_lifecycle_states(self) -> None:
        """Pickup-lifecycle values do not apply to plate_info catalog records."""
