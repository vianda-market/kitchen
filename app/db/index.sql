...
-- Partial unique index: canonical_key is unique when not null (sparse to avoid blocking ad-hoc credit currencies)
CREATE UNIQUE INDEX IF NOT EXISTS uq_credit_currency_canonical_key
    ON core.currency_metadata (canonical_key)
    WHERE canonical_key IS NOT NULL;
