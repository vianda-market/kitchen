...
'PUT /plate-kitchen-days/by-key. NULL for ad-hoc rows created by suppliers. '
'When set, must be UPPER_SNAKE_CASE and unique across all non-NULL rows '
'(enforced by the uq_plate_kitchen_days_canonical_key partial index).';

-- Mirror the column in the audit history table so triggers keep working.
ALTER TABLE audit.plate_kitchen_days_history
ADD COLUMN IF NOT EXISTS canonical_key VARCHAR(200) NULL;

COMMENT ON COLUMN audit.plate_kitchen_days_history.canonical_key IS
'Mirror of ops.plate_kitchen_days.canonical_key at the time of this history event.';
