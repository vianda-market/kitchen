...
def promote_plate_selection_to_live(
    plate_selection_id: UUID, system_user_id: UUID, db: psycopg2.extensions.connection, *, commit: bool = True
) -> UUID | None:
    """
    ...
    if existing:
        log_info(
            f"Plate selection {plate_selection_id} already promoted (plate_pickup_id={existing['plate_pickup_id']})"
        )
    ...
    if not credit_currency:
        log_error(
            f"Credit currency not found for restaurant {selection.restaurant_id}, promotion of {plate_selection_id}"
        )
        return None
    ...
    success = mark_plate_selection_complete(transaction_record.transaction_id, selection.user_id, db, commit=False)
    if not success:
...
def _create_pickup_record_for_promotion(
    selection: PlateSelectionDTO, context: dict[str, Any], modified_by: UUID, db: psycopg2.extensions.connection
) -> PlatePickupLiveDTO | None:
