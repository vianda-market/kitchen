...
from app.core.versioning import APIVersion, add_version_info_to_response, create_versioned_router, get_current_version
from app.schemas.versioned_schemas import get_schema_for_version
from app.services.crud_service import CRUDService
from app.services.route_factory import RouteConfig
...
def create_versioned_crud_routes(
    config: VersionedRouteConfig, service: CRUDService[T], version: APIVersion = None
) -> APIRouter:
    """
    ...
    def get_all_entities(current_version: APIVersion = Depends(get_current_version)):
    ...
    def get_entity(entity_id: UUID, current_version: APIVersion = Depends(get_current_version)):
    ...
    def create_entity(create_data: create_schema, current_version: APIVersion = Depends(get_current_version)):
    ...
    def update_entity(
        entity_id: UUID, update_data: update_schema, current_version: APIVersion = Depends(get_current_version)
    ):
    ...
    def delete_entity(entity_id: UUID, current_version: APIVersion = Depends(get_current_version)):
...
def create_versioned_plan_routes(version: APIVersion = APIVersion.V1) -> APIRouter:
    ...
    config = VersionedRouteConfig(
        prefix="plans", tags=["Plans"], entity_name="plan", entity_name_plural="plans", version=version
    )
...
def create_versioned_user_routes(version: APIVersion = APIVersion.V1) -> APIRouter:
    ...
    config = VersionedRouteConfig(
        prefix="users", tags=["Users"], entity_name="user", entity_name_plural="users", version=version
    )
