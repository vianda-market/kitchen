...
from app.config.archival_config import get_archival_priority_order, get_config_source
...
def run_daily_archival(batch_size: int = 200, max_tables: int = 15) -> dict[str, Any]:
    ...
    try:
        ...
        results = ArchivalService.run_scheduled_archival(
            modified_by=SYSTEM_USER_ID, batch_size=batch_size, max_tables=max_tables
        )
        ...
        if results.get("success", False):
            ...
            log_warning(
                f"Daily archival completed with errors: {total_archived} records archived, {len(errors)} errors"
            )
...
def run_archival_validation() -> dict[str, Any]:
    ...
    try:
        ...
        # Add cron job metadata
        validation_results.update({"cron_job": "archival_validation", "config_source": get_config_source()})
...
def get_archival_dashboard() -> dict[str, Any]:
    ...
    try:
        ...
        stats.update(
            {
                "dashboard_generated": datetime.now(UTC).isoformat(),
                "config_source": get_config_source(),
        ...
        log_error(error_msg)
        return {"dashboard_generated": datetime.now(UTC).isoformat(), "error": error_msg}
...
def run_priority_archival(priority_category: str = "financial_critical", batch_size: int = 50) -> dict[str, Any]:
    ...
    try:
        from app.config.archival_config import ArchivalCategory, get_tables_by_category
        ...
        results.update({"end_time": end_time.isoformat(), "success": len(results["errors"]) == 0})
