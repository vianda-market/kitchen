...
class CreditLoadingService:
    ...
    def create_client_credit_transaction(
        ...
        try:
            ...
            if subscription:
                ...
                if success:
                    log_info(
                        f"Updated subscription balance for user {user_id}: added {amount} credits via discretionary {discretionary_id}"
                    )
                else:
            ...
            log_info(
                f"Created client credit transaction: {client_transaction.transaction_id} for user {user_id} via discretionary {discretionary_id}"
            )
    ...
    def create_restaurant_credit_transaction(
        ...
        try:
            ...
            log_info(
                f"Created restaurant credit transaction: {restaurant_transaction.transaction_id} for restaurant {restaurant_id} via discretionary {discretionary_id}"
            )
