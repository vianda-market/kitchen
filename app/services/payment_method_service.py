...
def link_payment_method_to_type(
    payment_method_id: UUID, method_type: str, type_id: UUID, current_user_id: UUID, db: psycopg2.extensions.connection
) -> bool:
    """
    ...
    if payment_method.method_type != method_type:
        ...
        raise ValueError(
            f"Payment method type mismatch: expected '{method_type}', found '{payment_method.method_type}'"
        )
        ...
        with db.cursor() as cursor:
            query = """
                ...
                WHERE payment_method_id = %s
                  ...
                  AND status = 'pending'
            """
            cursor.execute(query, (str(type_id), str(current_user_id), str(payment_method_id), method_type))
            rows_updated = cursor.rowcount
            db.commit()

        if rows_updated > 0:
            log_info(f"Linked payment_method {payment_method_id} to {method_type} {type_id} and activated it")
            return True
        # This shouldn't happen given our checks above, but handle gracefully
        log_warning(f"Payment method {payment_method_id} update returned 0 rows (may have been updated concurrently)")
        return False
...
def create_payment_method_with_address(
    ...
    try:
        ...
        log_info(
            f"Payment method {payment_method.payment_method_id} created successfully with address {resolved_address_id}"
        )
