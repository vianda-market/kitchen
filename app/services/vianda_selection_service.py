# app/services/vianda_selection_service.py
"""
Business logic service for vianda selection operations.

This service handles the complex business logic for creating vianda selections,
...
from .vianda_selection_validation import (
...
def create_vianda_selection_with_transactions(
    ...
    """
    Create a vianda selection with all related transactions.

    This function orchestrates the entire vianda selection creation process,
    ...
    try:
        ...
        context = _fetch_vianda_selection_context(payload, db, locale)
        if not context:
            raise envelope_exception(ErrorCode.VIANDA_SELECTION_NOT_FOUND, status=400, locale=locale)
        ...
        # Step 2.7: Reject if user already has a vianda selection for this kitchen_day (or handle replace flow)
        ...
        # Step 4: Create the vianda selection (only if credits are sufficient)
        # Client transaction, subscription balance, vianda_pickup_live, and restaurant_transaction
        # are all created at kitchen_start by the promotion cron.
        selection = _create_vianda_selection_record(context, current_user, db, locale)
        if not selection:
            db.rollback()
            raise envelope_exception(
                ErrorCode.ENTITY_CREATION_FAILED, status=500, locale=locale, entity="vianda selection"
            )

        # Commit
        db.commit()
        log_info(
            f"Successfully created vianda selection {selection.vianda_selection_id} (vianda_pickup created at kitchen_start)"
        )
...
def _handle_duplicate_day(
    ...
    """
    Guard against duplicate vianda selections on the same kitchen day.

    When an existing selection is found:
    - If the caller sent replace_existing=True and the correct existing_vianda_selection_id,
    ...
    _existing = db_read(
        """
        SELECT vianda_selection_id FROM vianda_selection_info
    ...
    if not _existing:
        return
    existing_id = str(_existing["vianda_selection_id"])
    replace_existing = payload.get("replace_existing") is True
    payload_existing_id = payload.get("existing_vianda_selection_id")
    ...
    if replace_existing and payload_existing_id_str and payload_existing_id_str == existing_id:
        ...
        cancel_vianda_selection(UUID(existing_id), current_user, db, commit=False)
    else:
        # Duplicate without valid replace: return structured 409 for frontend modal
        raise envelope_exception(
            ErrorCode.VIANDA_SELECTION_DUPLICATE_KITCHEN_DAY,
            status=409,
            locale=locale,
...
def _apply_early_renewal_if_needed(
    ...
    swallowed so the vianda selection flow continues.
    """
    from fastapi import HTTPException
...
def _assert_sufficient_credits(
    ...
    """
    Raise HTTP 402 if the user's credit balance is insufficient for the vianda's credit cost.
    Must be called BEFORE any records are inserted.
    """
    from fastapi import HTTPException

    credit_validation = validate_sufficient_credits(current_user["user_id"], context["vianda"].credit, db)
    if not credit_validation.has_sufficient_credits:
        insufficient_credits_response = handle_insufficient_credits(
            current_user["user_id"], context["vianda"].credit, credit_validation.current_balance
        )
        raise HTTPException(
...
def _fetch_vianda_selection_context(
    ...
    """
    Fetch all required data for vianda selection in one place.
    ...
    """
    try:
        vianda_id = UUID(str(payload["vianda_id"]))
    except (ValueError, KeyError):
        raise envelope_exception(ErrorCode.DATABASE_INVALID_UUID, status=400, locale=locale) from None

    # Fetch vianda
    vianda = vianda_service.get_by_id(vianda_id, db)
    if not vianda:
        raise envelope_exception(ErrorCode.ENTITY_NOT_FOUND, status=404, locale=locale, entity="Vianda")
    ...
    # Fetch QR code
    qr_code = qr_code_service.get_by_restaurant(vianda.restaurant_id, db)
    if not qr_code:
    ...
    # Fetch kitchen days for the vianda
    # Note: We pass scope=None to get all kitchen days (not filtered by institution)
    # This is correct for vianda selection, as customers should see all available days for a vianda
    # The filtering by vianda_id happens below
    from app.services.crud_service import vianda_kitchen_days_service

    kitchen_days = vianda_kitchen_days_service.get_all(db, scope=None, include_archived=False)
    vianda_kitchen_days = [kd.kitchen_day for kd in kitchen_days if kd.vianda_id == vianda_id]
    ...
    return {
        ...
        "kitchen_days": vianda_kitchen_days,
        "payload": payload,
    }


def _create_vianda_selection_record(
    context: dict[str, Any], current_user: dict[str, Any], db: psycopg2.extensions.connection, locale: str = "en"
) -> ViandaSelectionDTO:
    """
    Create the main vianda selection record.

    Raises:
        HTTPException: If vianda selection creation fails
    """
    ...
    target_date = context.get("target_date")
    if not target_date:
        raise envelope_exception(ErrorCode.ENTITY_CREATION_FAILED, status=500, locale=locale, entity="vianda selection")
    ...
    selection = vianda_selection_service.create(data, db, commit=False)
    if selection:
        log_info(f"Created vianda selection: {selection.vianda_selection_id} (commit deferred)")
        return selection
    raise envelope_exception(ErrorCode.ENTITY_CREATION_FAILED, status=500, locale=locale, entity="vianda selection")


# Fields that may NOT be modified via PATCH. To change vianda, user must cancel and create new.
...
def update_vianda_selection(
    ...
    """
    Update a vianda selection. Only pickup_time_range, pickup_intent, flexible_on_time,
    and cancel are editable. vianda_id and other fields cannot be changed; user must
    cancel and create a new selection to change the vianda.
    Validates editability window before allowing updates.
    """
    from app.services.kitchen_day_service import is_vianda_selection_editable
    ...
    if forbidden_in_payload:
        raise envelope_exception(
            ErrorCode.VIANDA_SELECTION_IMMUTABLE_FIELDS,
            status=400,
            locale=locale,
    ...
    if not selection:
        raise envelope_exception(ErrorCode.VIANDA_SELECTION_NOT_FOUND, status=404, locale=locale)
    ...
    if not is_vianda_selection_editable(vianda_selection_id, db):
        raise envelope_exception(ErrorCode.VIANDA_SELECTION_NOT_EDITABLE, status=400, locale=locale)
    ...
    if "pickup_intent" in payload and payload["pickup_intent"] is not None:
        ...
        if intent not in ("offer", "request", "self"):
            raise envelope_exception(ErrorCode.VIANDA_SELECTION_PICKUP_INTENT_INVALID, status=422, locale=locale)
    ...
    if payload.get("cancel") is True:
        return cancel_vianda_selection(vianda_selection_id, current_user, db, locale=locale)

    if not updates:
        return selection

    updates["modified_by"] = current_user["user_id"]
    updated = vianda_selection_service.update(vianda_selection_id, updates, db)
    if not updated:
        raise envelope_exception(ErrorCode.ENTITY_UPDATE_FAILED, status=500, locale=locale, entity="vianda selection")
    log_info(f"Updated vianda selection {vianda_selection_id}")
    return updated


def cancel_vianda_selection(
    vianda_selection_id: UUID,
    current_user: dict[str, Any],
    db: psycopg2.extensions.connection,
    commit: bool = True,
    ...
    """
    Cancel a vianda selection: soft-delete selection and related records.
    Allowed only until 1 hour before kitchen day (is_vianda_selection_editable).
    ...
    if not is_vianda_selection_editable(vianda_selection_id, db):
        raise envelope_exception(ErrorCode.VIANDA_SELECTION_NOT_CANCELLABLE, status=400, locale=locale)
    ...
    try:
        ...
        # Cancellation is only allowed before lock (is_vianda_selection_editable enforces this).
        ...
        vianda_selection_service.soft_delete(vianda_selection_id, current_user["user_id"], db)
        ...
        for row in pickup_rows or []:
            vianda_pickup_live_service.soft_delete(row["vianda_pickup_id"], current_user["user_id"], db)

        if commit:
            db.commit()
        log_info(f"Cancelled vianda selection {vianda_selection_id}")
        ...
        if isinstance(e, HTTPException):
            ...
            raise
        db.rollback()
        log_error(f"Error cancelling vianda selection: {e}")
        raise envelope_exception(
            ErrorCode.ENTITY_DELETION_FAILED, status=500, locale=locale, entity="vianda selection"
        ) from None


def delete_vianda_selection(
    vianda_selection_id: UUID, current_user: dict[str, Any], db: psycopg2.extensions.connection, locale: str = "en"
) -> dict[str, str]:
    """Delete (cancel) a vianda selection. Same as cancel - soft-deletes selection and archives."""
    cancel_vianda_selection(vianda_selection_id, current_user, db, locale=locale)
    return {"detail": "Vianda selection deleted successfully"}
