...
from app.utils.db import db_batch_delete, db_read
...
class ArchivalService:
    ...
    def get_eligible_records(cls, table_name: str, limit: int = 1000) -> list[dict[str, Any]]:
        ...
        try:
            ...
            cutoff_date = datetime.now(UTC) - timedelta(days=config.retention_days + config.grace_period_days)
            ...
            if not model_class:
                ...
                return [dict(zip([desc[0] for desc in results], row, strict=False)) for row in results]
    ...
    def archive_records(cls, table_name: str, record_ids: list[UUID], modified_by: UUID) -> dict[str, Any]:
        ...
        try:
            ...
            get_table_archival_config(table_name)
            model_class = cls._get_model_class(table_name)
            ...
            try:
                ...
                soft_update_fields = {"modified_by": str(modified_by), "modified_date": datetime.now(UTC)}
                ...
                archived_count = db_batch_delete(
                    table_name, where_list, connection=conn, soft=True, soft_update_fields=soft_update_fields
                )
            ...
            result = {"archived_count": archived_count, "total_requested": len(record_ids), "table_name": table_name}
    ...
    def run_scheduled_archival(cls, modified_by: UUID, batch_size: int = 100, max_tables: int = 10) -> dict[str, Any]:
        ...
        try:
            ...
            results = {"start_time": start_time.isoformat(), "tables_processed": [], "total_archived": 0, "errors": []}
                ...
                try:
                    ...
                    if not eligible_records:
                        ...
                        results["tables_processed"].append(
                            {"table_name": table_name, "eligible_count": 0, "archived_count": 0}
                        )
                        continue
                    ...
                    results["tables_processed"].append(
                        {"table_name": table_name, "eligible_count": 0, "archived_count": 0, "error": str(e)}
                    )
            ...
            results.update(
                {"end_time": end_time.isoformat(), "duration_seconds": duration, "success": len(results["errors"]) == 0}
            )

            log_info(
                f"Scheduled archival completed. Total archived: {results['total_archived']}, Duration: {duration:.2f}s"
            )
            return results
        ...
        try:
            ...
            for table_name in priority_tables:
                try:
                    ...
                    try:
                        eligible_count = (
                            int(eligible_result[0])
                            if eligible_result
                            and isinstance(eligible_result, (tuple, list))
                            and len(eligible_result) > 0
                            else 0
                        )
                    ...
                    stats["table_stats"].append({"table_name": table_name, "error": str(e)})
    ...
    def validate_archival_integrity(cls) -> dict[str, Any]:
        ...
        try:
            ...
            return {"timestamp": datetime.now(UTC).isoformat(), "overall_status": "error", "fatal_error": str(e)}
