...
class GoogleMapsGateway(BaseGateway):
    ...
    def reverse_geocode(self, latitude: float, longitude: float) -> str:
        ...
        if not response.get("results"):
            raise ExternalServiceError(f"No address found for coordinates: {latitude}, {longitude}")

        return response["results"][0]["formatted_address"]

    def get_address_components(self, address: str) -> dict[str, str]:
        """
...
# Singleton instance
_google_maps_gateway: GoogleMapsGateway | None = None
