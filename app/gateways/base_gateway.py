...
class BaseGateway(ABC):
    ...
    def _get_mock_response(self, operation: str, **kwargs) -> Any:
        ...
        if not self._mock_data or operation not in self._mock_data:
            ...
            raise ExternalServiceError(f"Mock response not configured for operation '{operation}'")

        logger.info(f"🎭 Returning mock response for {self.service_name}.{operation}")
        return self._mock_data[operation]

    def _log_api_call(
        self, operation: str, start_time: datetime, success: bool, error: str | None = None, **kwargs
    ) -> None:
        """
    ...
    def _load_mock_file(self, filename: str) -> dict[str, Any]:
        ...
        if not mock_path.exists():
            ...
            raise ExternalServiceError(f"Mock file '{filename}' not found. Create it in app/mocks/")
        ...
        try:
            ...
            raise ExternalServiceError(f"Invalid JSON in mock file '{filename}': {str(e)}") from e
